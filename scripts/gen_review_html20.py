#!/usr/bin/env python3
"""
Form 20 physician-review HTML (offline, gitignored). Differs from the Form 31 generator:
 - NO official NBME explanation exists -> show the previous test-taker's UNVERIFIED note instead.
 - Answer-key review is first-class: each card shows how the answer was marked + a confidence
   chip, and a top banner lists the ADJUDICATED / low-confidence answers (e.g. q164 C->A).
 - Each enrichment shows its self-reported confidence + any verify_note, so the physician's
   attention goes where the model is least sure.
 - localStorage keyed to a build id (family 'f20-') so a regenerated set starts clean.
"""
...
O = ROOT / "import/out20"
OUT = ROOT / "enrichment_review_form20.html"

items = {}
for m in sorted(glob(str(O / "block*.merged.json"))):
    for it in json.loads(pathlib.Path(m).read_text())["items"]:
        items[it["q_number"]] = it
enr = {}
for b in range(1, 11):
    f = O / f"enrich_block{b}_full.json"
    if f.exists():
        for e in json.loads(f.read_text())["enrichments"]:
            enr[e["q_number"]] = e

QLIST = sorted(q for q in items if q in enr)
BUILD = "f20-" + hashlib.sha1(json.dumps(enr, sort_keys=True, ensure_ascii=False).encode()).hexdigest()[:8]
FAMILY = "f20-"

g = sum(1 for e in enr.values() for x in e.get("high_yield", []) + e.get("how_they_test", []) if x.get("source") != "model")
m = sum(1 for e in enr.values() for x in e.get("high_yield", []) + e.get("how_they_test", []) if x.get("source") == "model")

# flag sets
ADJ = {q for q, it in items.items() if it.get("adjudication")}
LOWANS = {q for q, it in items.items() if it.get("answer_confidence") == "low"}
NEEDIMG = {q for q, it in items.items() if it.get("needs_image")}
LOWENR = {q for q, e in enr.items() if e.get("confidence") == "low"}
VERIFY = {q for q, e in enr.items() if (e.get("verify_note") or "").strip()}
...
def card(n):
    ...
    knocks = "".join(f'<li><b>{md(k.get("option",""))}</b> — {md(k.get("reason",""))}</li>' for k in e.get("knockdowns", []))
    highs = "".join(f'<li>{md(x.get("fact",""))} {src_chip(x.get("source"))}</li>' for x in e.get("high_yield", []))
    tests = "".join(f'<li><span class="scn">{md(x.get("scenario",""))}</span> → <b>{md(x.get("answer",""))}</b> '
                    f'{src_chip(x.get("source"))}</li>' for x in e.get("how_they_test", []))
    badges = ""
    if n in ADJ:   badges += '<span class="badge err">⚑ ANSWER CHANGED — confirm</span>'
    if n in LOWANS and n not in ADJ: badges += '<span class="badge err">⚑ low-confidence answer</span>'
    if n in NEEDIMG: badges += '<span class="badge img">🖼 answer depends on the figure</span>'
    if it.get("clinical_image_url"): badges += '<span class="badge fig">🖼 has clinical image</span>'
    if n in LOWENR: badges += '<span class="badge warn">enrichment: low confidence</span>'
    conf = e.get("confidence", "?")
    marking = esc(it.get("answer_marking", "") or it.get("answer_marking", ""))
    adj = f'<div class="adj">⚑ {esc(it["adjudication"])}</div>' if it.get("adjudication") else ""
    vnote = f'<div class="vnote">🔎 <b>Verify:</b> {esc(e["verify_note"])}</div>' if (e.get("verify_note") or "").strip() else ""
    snote = esc(it.get("student_note", "") or "(none)")
...
<article class="card" id="q{n}" data-q="{n}" data-verdict="">
  ...
  <div class="correct">Correct answer &nbsp;<b>{esc(it["correct_letter"])} — {esc(correct)}</b>
    <span class="mark">marked via {marking or "?"} · confidence {esc(it.get("answer_confidence","?"))}</span></div>
  {adj}
  <div class="layer lock"><h3>Bottom line</h3><p>{md(e.get("answer_lock",""))}</p></div>
  <div class="layer knock"><h3>Knockdowns</h3><ul>{knocks}</ul></div>
  <div class="layer hook"><h3>Remember it as</h3><p>{md(e.get("hook",""))}</p></div>
  <div class="layer hy"><h3>High yield</h3><ul>{highs}</ul></div>
  <div class="layer test"><h3>How they test it</h3><ul>{tests}</ul></div>
  {vnote}
  <div class="enrconf">enrichment self-confidence: <b>{esc(conf)}</b></div>
  <details class="collapse source"><summary>Previous test-taker's note <span class="hint">(UNVERIFIED — not NBME; may be wrong)</span></summary>
    <div class="pre">{snote}</div></details>
...
adj_chips = " ".join(f'<a class="chip" href="#q{q}">Q{q}</a>' for q in sorted(ADJ | LOWANS))
verify_chips = " ".join(f'<a class="chip warn" href="#q{q}">Q{q}</a>' for q in sorted(VERIFY | LOWENR))
img_chips = " ".join(f'<a class="chip img" href="#q{q}">Q{q}</a>' for q in sorted(NEEDIMG))
QLIST_JS = json.dumps(QLIST)

TEMPLATE = r"""<!doctype html>
<html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover">
<title>Form 20 — Enrichment Review</title>
<style>
  ...
  *{box-sizing:border-box}
  ...
  .banner.err .chip{background:#ef4444}
  .banner.warn{background:#fffbeb;border-color:#fde68a;color:#92400e} .banner.warn b{color:#92400e}
  .banner.warn .chip.warn{background:#f59e0b}
  .chip.img{background:#6366f1}
  ...
  .badge.err{background:#fee2e2;color:#b91c1c} .badge.img{background:#e0e7ff;color:#3730a3}
  .badge.fig{background:#ecfeff;color:#0e7490} .badge.warn{background:#fef3c7;color:#92400e}
  ...
  .correct .mark{display:block;color:#3f6151;font-size:.8rem;font-weight:600;margin-top:3px}
  .adj{background:#fef2f2;border:1px solid #fecaca;color:#991b1b;border-radius:10px;padding:9px 13px;margin:8px 0;font-size:.9rem;font-weight:600}
  .layer{margin:14px 0}
  ...
  .vnote{background:#fffbeb;border:1px solid #fde68a;color:#92400e;border-radius:10px;padding:9px 13px;margin:10px 0;font-size:.9rem}
  .enrconf{color:var(--muted);font-size:.82rem;margin:6px 0}
...
<header class="top"><div class="wrap">
  <h1>NBME Form 20 · Enrichment Review <span style="opacity:.55;font-size:.7rem;font-weight:500">build __BUILD__</span></h1>
  <div class="progress"><span id="progress">0 of __TOTAL__ reviewed</span><span class="bar"><i id="progressBar"></i></span></div>
</div></header>
<div class="wrap">
  <div class="notices">
    <div class="banner err"><b>🔴 Answer key — check these first.</b> No official NBME key exists for this form;
      each answer was read visually and independently double-checked. These items were changed or are
      low-confidence — please confirm: __ADJCHIPS__</div>
    <div class="banner warn"><b>🟡 Enrichment flags to verify:</b> items the model marked low-confidence or
      attached a "verify" note to (no NBME explanation backs this form — scrutinize these): __VERIFYCHIPS__</div>
    <div class="note">__TOTAL__ questions. There is <b>no NBME explanation</b> for Form 20 — enrichment is grounded
      in re-chunked Mehlman where possible (a <span class="src grounded">green citation</span>) or
      <span class="src model">model-generated</span> otherwise (__G__ grounded / __M__ model facts) — scrutinize amber.
      The collapsed note per card is the <b>previous test-taker's own scribble (unverified)</b>, shown only so you can
      compare. 🖼 items depend on a figure: __IMGCHIPS__. Progress saves on this device.</div>
  </div>
  __CARDS__
  <footer class="bottom"><h2>Export review</h2>
    <p style="margin:0 0 12px;color:#5b6774;font-size:.95rem">Export your verdicts + comments and send the file back.</p>
...
<script>
  ...
  function buildJSON(){const reviews=QLIST.map(q=>{const st=state[q]||{};
    ...
    return JSON.stringify({form:20, enrichment_version:BUILD, exported_at:new Date().toISOString(),
  ...
  document.getElementById('export').addEventListener('click',()=>{const out=buildJSON();const ta=document.getElementById('exportOut');
    ...
    try{const blob=new Blob([out],{type:'application/json'});const url=URL.createObjectURL(blob);const a=document.createElement('a');
      a.href=url;a.download='enrichment_review_form20_'+BUILD+'_results.json';a.click();URL.revokeObjectURL(url);flash('Downloaded ✓');}
...
OUT.write_text(TEMPLATE.replace("__CARDS__", CARDS).replace("__BUILD__", BUILD)
               ...
               .replace("__QLIST__", QLIST_JS).replace("__ADJCHIPS__", adj_chips or "(none)")
               .replace("__VERIFYCHIPS__", verify_chips or "(none)").replace("__IMGCHIPS__", img_chips or "(none)")
               .replace("__G__", str(g)).replace("__M__", str(m)))
print(f"wrote {OUT.name} ({OUT.stat().st_size//1024} KB) — {len(QLIST)} questions, build {BUILD}, grounded {g}/model {m}")
print(f"  adjudicated/low-answer: {sorted(ADJ|LOWANS)}")
print(f"  verify/low-enrichment:  {sorted(VERIFY|LOWENR)}")
print(f"  needs-image: {sorted(NEEDIMG)}")
