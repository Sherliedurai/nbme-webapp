  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-8">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        {/* Self-Reflections */}
        {(() => {
          const reflections: Array<{ qId: number; topic: string; whyWrong: string }> = [];
          examData.questions.forEach((q, i) => {
            const blockIdx = Math.floor(i / QUESTIONS_PER_BLOCK);
            const qs = blocks[blockIdx]?.questionStates[q.id];
            if (qs?.whyWrong) {
              reflections.push({ qId: q.id, topic: q.topic || 'Uncategorized', whyWrong: qs.whyWrong });
            }
          });
          if (reflections.length === 0) return null;
          // Group by reflection text
          const grouped: Record<string, string[]> = {};
          reflections.forEach((r) => {
            const key = r.whyWrong.toLowerCase().trim();
            if (!grouped[key]) grouped[key] = [];
            grouped[key].push(r.topic);
          });
          return (
            <div className="bg-card rounded-lg border p-4">
              <h2 className="font-semibold mb-3">🪞 Self-Reflections</h2>
              <div className="space-y-2">
                {Object.entries(grouped)
                  .sort((a, b) => b[1].length - a[1].length)
                  .map(([reason, topics], idx) => (
                    <div key={idx} className="flex items-start gap-2 text-sm">
                      <span className="text-muted-foreground shrink-0">×{topics.length}</span>
                      <span className="italic text-foreground">"{reason}"</span>
                      <span className="text-muted-foreground text-xs ml-auto">
                        {[...new Set(topics)].slice(0, 3).join(', ')}
                      </span>
                    </div>
                  ))}
              </div>
