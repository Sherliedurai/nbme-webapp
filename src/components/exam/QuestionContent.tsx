...
const QuestionContent: React.FC = () => {
  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 md:px-6 py-4 max-w-3xl mx-auto">
      ...
      {/* Add to Review Deck — show for ALL answered questions */}
      ...
      {showFeedback && !isCorrectAnswer && !isReview && (
        <div className="mb-3">
          <label className="text-xs font-medium text-muted-foreground mb-1 block">Why did I get this wrong?</label>
          <input
            type="text"
            value={questionState?.whyWrong ?? ''}
            onChange={(e) => dispatch({ type: 'SET_WHY_WRONG', payload: { questionId: question.id, whyWrong: e.target.value } })}
            placeholder="Confused X with Y / Didn't read stem carefully / Never learned this"
            className="w-full h-8 px-3 text-xs rounded-md border border-input bg-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
          />
        </div>
      )}
