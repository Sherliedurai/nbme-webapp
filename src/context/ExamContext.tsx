...
type Action =
  ...
  | { type: 'SET_WHY_WRONG'; payload: { questionId: number; whyWrong: string } }
...
function examReducer(state: ExamState, action: Action): ExamState {
  switch (action.type) {
          ...
          i === state.currentBlock
            ? {
                ...
                questionStates: {
                  ...
                  [action.payload.questionId]: {
                    ...b.questionStates[action.payload.questionId],
                    whyWrong: action.payload.whyWrong,
                  },
                },
              }
            : b
    ...
    case 'END_BLOCK': {
      ...
      const showRecap = state.instantFeedback;
      return {
        ...state,
        phase: isLastBlock ? 'complete' : (showRecap ? 'blockRecap' : 'break'),
        paused: false,
        blocks: state.blocks.map((b, i) =>
        ...
        if (parsed.examData && parsed.phase !== 'start') {
          ...
          Object.values(parsed.blocks).forEach(block => {
            Object.values(block.questionStates).forEach((qs: any) => {
              ...
              if (qs.whyWrong === undefined) qs.whyWrong = '';
              if (qs.streak === undefined) qs.streak = 0;
            });
          });
